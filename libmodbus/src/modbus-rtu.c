...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    case 115200:
        ...
        break;
    case 1152000:
        dcb.BaudRate = CBR_1152000;
        break;
    case 1250000:
        dcb.BaudRate = CBR_1250000;
        break;
    default:
        dcb.BaudRate = CBR_9600;
    ...
    */
    ...
    case 1152000:
        ...
        break;    
    case 1250000:
        ioctl(ctx->s, TIOCGSERIAL, &ss);
        ss.flags = (ss.flags & ~ASYNC_SPD_MASK) | ASYNC_SPD_CUST;
        ss.custom_divisor = (ss.baud_base + (ctx_rtu->baud / 2)) / ctx_rtu->baud;
        int closestSpeed = ss.baud_base / ss.custom_divisor;

        if (closestSpeed < ctx_rtu->baud * 98 / 100 || closestSpeed > ctx_rtu->baud * 102 / 100) {
            fprintf(stderr, "Cannot set serial port speed to. Closest possible is %i\n", closestSpeed);
        }

        ioctl(ctx->s, TIOCSSERIAL, &ss);
        speed = B38400;
    ...
    fprintf(stderr, "Setting speed to: %i \n",  speed);
    /* Set the baud rate */
...
#if HAVE_DECL_TIOCSRS485
    fprintf(stderr, "HAVE_DECL_TIOCSRS485\n");
    ...
    if (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU) {
        ...
        if (mode == MODBUS_RTU_RS485) {
            ...
            if (ioctl(ctx->s, TIOCGRS485, &rs485conf) < 0) {
                fprintf(stderr, "Getting serial mode failed\n");
                return -1;
            }
            // Set
            rs485conf.flags |= SER_RS485_ENABLED;
            if (ioctl(ctx->s, TIOCSRS485, &rs485conf) < 0) {
                fprintf(stderr, "Setting serial mode to rs485 failed\n");
                return -1;
            }
            ...
            fprintf(stderr, "Setting to serial mode RTU RS232\n");
            ctx_rtu->serial_mode = MODBUS_RTU_RS232;
            return 0;
        }
#else
